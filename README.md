
# MoveApps: Collar Health Shiny App
Github repository: *github.com/sitkensis22/Collar-Health-Shiny-App* (*https://github.com/sitkensis22/Collar-Health-Shiny-App*)

<img width="1916" height="1038" alt="Screenshot 2026-06-02 at 3 02 51 PM (2)" src="https://github.com/user-attachments/assets/429971d7-3078-4f8e-a1d4-f5a83b04d423" />

## Description
Provides visualization tools for alerts generated by Collar-Health-Alert-App (and possibly filtered by Collar-Health-Filter-App) including Leaflet map, individual and all tabular data, and alert-specific plots. The user can (1) select data to view by individual and filter by those with or without alerts and by status (e.g., dead, active); (2) select alert events; (3) select one of 6 basemaps; (4) filter data by date or number of locations; (5) remove individuals from the dataset; (6) link to the study's Movebank deployment page; (7) download individual data, all data, an individual report that contains summary, map, and location data, or the individual or all location data in shapefile or KML file format (depending on the map_all toggle switch); (8) and select which data fields to display in individual data table.  

## Documentation
This App creates a Shiny UI that allows the interactive visualization of move2 location data in map, graphical, and tabular form. The App is meant to work in conjunction in a workflow with the Collar-Health-Alert-App (and possibly filtered by Collar-Health-Filter-App) and uses the move2 dataset with fields appended for each alert type: `mortality`, `resurrection`, `cluster`, `nsd`, `voltage`, `gps_accuracy`, `gps_transmission`, `gps_resurrection`, and `tag_release`, the number of unique alerts per individual, `nAlerts`, and the distance between consecutive locations (`distMoved`). The App will not work without these fields appended to the dataset. The alerts are binary fields (0/1) indicating which locations meet the one of 9 alert criteria, and these locations are displayed in orange color on the Leaflet map. The start and end of the movement track are colored green and red, respectively, on the Leaflet map. The user can select individuals by clicking on the "Track information" table, the "All data" table, or in the "Select individual" dropdown menu. The "Select alert" dropdown menu will automatically update as different individuals are selected, as will the date range, number of locations, "Individual data" table, and Event plots. The Leaflet map locations will show the datetime when the user hovers over the points, and tooltips will also be displayed on the Event plots as the user hovers over the points. Note that the "device_id" field is the `tag_local_identifier` (or `serial_no`) field in the track data.

### Application scope
#### Generality of App usability
This app was developed for any taxinomic group.  

#### Required data properties
The App should work for any kind of (location) data. However, the App is only meant to work in conjunction with the Collar Health Alert App in a previous step of workflow.

### Input type
`move2::move2_loc`

### Output type
`move2::move2_loc`

### Artefacts
None.

The Download button enables a local download of the "All data" table (as a .csv file), "Individual data" table (as a .csv file), or collar health report (as an HTML file) depending on the `Select output` input. 

### Settings 

**Alert toggle (`alert_toggle`):** This switch filters the individuals in the data based on if they have any alerts or not. Defaults to selecting all individuals. 

**Status toggle (`status_toggle`):** This switch filters the individuals in the data based on their status (either "active" or another level based on the `deployment_end_type` field in the track data). Defaults to selecting all individuals. 

**All map toggle (`map_all`):** This switch switches between showing all individuals on the map and the selected individual. Defaults to displaying the selected individual. 

**Select individual (`individual_select`):** Select the individual based on the track id column in the move2 dataset. Note that the individual can also be selected by clicking on "Track information" or "All data" table rows. Defaults to the first id in the move2 dataset.

**Select alert (`notification_type`):** Select the alert to display in the Leaflet map, "Individual data" table, and Event plot tabs. Note that this dropdown menu will automatically update based on the selected individual. Possible values are: mortality, resurrection, cluster, nsd, voltage, gps_accuracy, gps_transmission, and gps_resurrection.

**Select basemap (`basemap_type`):** Select provider basemap for Leaflet map tab. Options include: World Imagery, World Topo Map, World Street Map, NatGeo World Map, OpenStreet Map, and OpenStreet Topo Map. Default is World Imagery.

**Data filter toggle (`filter_toggle`):** This switch filters the individual's data by either a date range or number of locations. Depending on the position of the toggle, a UI element will appear for either the date range or number of location data filters. Note that this toggle only appears when the map_all input toggle is turned off. Default is to filter by date range. 

**Date range filter (`date_range`):** Select the date range to filter the move2 dataset by. Note that the available date range will automatically update when different individuals are selected, and that widget only appears when the map_all input toggle is turned off. Default is the entire available date range. 

**Number of locations filter (`number_locations`):** Select the number of locations to filter the move2 dataset by. Note that the maximum number of locations will automatically update when different individuals are selected, and that widget only appears when the map_all input toggle is turned off. Default is the maximum number of locations.

**Data filter toggle (`filter_toggle_all`):** This switch filters the all data by either a date range or number of locations. Depending on the position of the toggle, a UI element will appear for either the date range or number of location data filters. Note that this toggle only appears when the map_all input toggle is turned on. Default is to filter by date range. 

**Date range filter (`date_range_all`):** Select the date range to filter the entire move2 dataset by. Note that the available date range will automatically update for the entire dataset, and that widget only appears when the map_all input toggle is turned on. Default is the entire available date range.

**Number of locations filter (`number_locations_all`):** Select the number of locations to filter the entire move2 dataset by. Note that the maximum number of locations will automatically update for the entire dataset, and that widget only appears when the map_all input toggle is turned on. Default is the maximum number of locations.

**Remove individual (`delete`):** This button will remove the currently selected individual from the dataset. This feature was provided for data editing purposes where an individual's deployment was ended on Movebank and the user does not want to consider this indvidual for further inspection.

**Open Movebank (`movebank_link`):** This button will open a new webpage tab of the study deployment page on Movebank. Note that the user must be logged into Movebank for this page to appear properly, which is recommended before the App is run and data editing commences. 

**Select output (`download_select`):** Select the local download of the "All data" table (as a .csv file), "Individual data" table (as a .csv file), collar health report (as an HTML file), or the location data as a KML file or a zipped folder of shapefiles, with respective options being "All table", "Individual table", "Report", "KML", or "Shapefile". Note that when the map_all input is switched off, the KML and shapefiles will be for the selected individual, and when it is switched on, the KML and shapefiles will be for all individuals. Default is "All table". 

**Download (`downloadData`):** This button will initiate the local download that is selected from the `download_select` dropdown menu.

**Show data fields toggle (`data_toggle`):** This switch will toggle the check boxes to select data fields to display in the "Individual data" table. Default is to show to available data fields. 

**Select data fields (`data_fields`):** Select the data fields to display in the "Individual data" table. Defaults to the track id, device_id, Latitude, Longitude, timestamp, alert (currently selected), and mortality_status. 

**Store UI settings (`Store settings`):** Click to store the current settings of the App for future Workflow runs. 

### Changes in output data
The input data remains unchanged.

### Most common errors
Please document and send errors to daniel.eacker@tauruswildlifeconsulting.com.
